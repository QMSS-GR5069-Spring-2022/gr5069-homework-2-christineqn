...
study1b_data <- study1b_data %>% filter(Status==0) #remove non-data rows (variable info + test rows)
...
study1b_data <- study1b_data %>%
  mutate(CWVmanip = ifelse(CWVmanip=='CWVmaniplow',0,1),
         targetgender = ifelse(target=='Jennifer',2,1))
...
study1b_onlymed <- study1b_data %>% filter(cat_selfbell == 'med')
...
mean(study1b_data$age[study1b_data$age<150]); sd(study1b_data$age[study1b_data$age<150])
