...
manipCWVpilot.2_data <- manipCWVpilot.2_data %>%
  filter(entered_mTurkID == workerId & attencheck_1 == 5)
...
manipCWVpilot.2_data <- manipCWVpilot.2_data %>%
  ...
  mutate(bell_behav = mean(c(harshlanguage, showdisgust, aggressivegestures,
                             actintimidate, readyupset, actcold, threats, blame)), warm_behav = mean(c(apologize,
                                                                                                       actnice, polite, showconcern))) %>%
  ungroup

# other constructs ####
manipCWVpilot.2_data <- manipCWVpilot.2_data %>%
  rowwise() %>%
  mutate(self_bell = mean(c(self_dominant, self_forceful, self_cold,
                            6 - self_warm)), comprehension = ifelse(questioneasy >= 3 & thinkeasy >=
                                                                      2, 1, 0)) %>%
  ungroup()
...
manipCWVpilot.2_onlymed <- manipCWVpilot.2_data %>%
...
getlms <- function(IV, coeff) {
  ...
  for (i in 1:length(behaviors)) {
    estimates <- append(estimates, summary(lms[[i]])$coefficients[coeff,
                                                                  1] %>%
                          round(3))
    ps <- append(ps, summary(lms[[i]])$coefficients[coeff, 4] %>%
                   round(3))
  }
  df <- data.frame(term = behaviors, estimate = estimates, p = ps, stars = ifelse(ps <
                                                                                    0.001, "***", ifelse(ps < 0.01, "**", ifelse(ps < 0.05, "*", ifelse(ps <
                                                                                                                                                          0.1, ".", "")))), stringsAsFactors = F)
  df <- df %>%
    rename_with(~paste(., IV), -term)
  return(df)
}
...
summary(lm(bell_behav ~ condition, manipCWVpilot.2_data %>%
             filter(comprehension == 1)))
summary(lm(warm_behav ~ condition, manipCWVpilot.2_data %>%
             filter(comprehension == 1)))
