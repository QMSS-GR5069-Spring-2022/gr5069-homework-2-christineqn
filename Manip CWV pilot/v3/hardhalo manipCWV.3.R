...
manipCWV.3_data <- manipCWV.3_data %>%
  filter(entered_mTurkID == workerId & attencheck_1 == 2 & attencheck_2 ==
           2)
...
# constructing eval #### alpha(select(manipCWV.3_data, c(competent,
# intelligent)), check.keys=T) alpha(select(manipCWV.3_data,
...
manipCWV.3_onlymed <- manipCWV.3_data %>%
  filter(cat_selfbell == "med")
# nrow(manipCWV.3_onlymed) #107
manipCWV.3_onlyeasy <- manipCWV.3_data %>%
  filter(questioneasy > 2 & thinkeasy > 1)
...
mean(manipCWV.3_data$age[manipCWV.3_data$age < 150])
sd(manipCWV.3_data$age[manipCWV.3_data$age < 150])
