---
title: "hardhalo study1b pilot-V4"
...
# drop incomplete cases #### sum(manipCWV.4_data$Finished!=1)
...
manipCWV.4_data <- manipCWV.4_data %>%
  filter(entered_mTurkID == workerId & attencheck_1 == 2)
# nrow(manipCWV.4_data) #300
manipCWV.4_data <- manipCWV.4_data %>%
  filter((condition == "competitive" & howact > 3) | (condition == "cooperative" &
                                                        howact < 3)) %>%
  filter((condition == "competitive" & whatwrite == 1) | (condition ==
                                                            "cooperative" & whatwrite == 2))
# nrow(manipCWV.4_data) #251

# calculate mean CWV #### R=2,5,7,9,10 alpha(select(manipCWV.4_data,
# c(CWV_1:CWV_10)), check.keys=T) #0.7
manipCWV.4_data <- manipCWV.4_data %>%
  rowwise() %>%
...
# constructing eval #### alpha(select(manipCWV.4_data, c(competent,
# intelligent)), check.keys=T) alpha(select(manipCWV.4_data,
# c(leader, negotiator, solver)), check.keys=T)
manipCWV.4_data <- manipCWV.4_data %>%
  rowwise() %>%
...
# write.csv(manipCWV.4_data, file='manipCWV.4_processed.csv',
# row.names=F) manipCWV.4_data <-
# read.csv('manipCWV.4_processed.csv') %>% mutate(cat_selfbell =
# factor(cat_selfbell, levels=c('low','med','high')))
...
mean(manipCWV.4_data$age[manipCWV.4_data$age < 150])
sd(manipCWV.4_data$age[manipCWV.4_data$age < 150])
...
summary(lm(gen_competent ~ condition * howrecent, manipCWV.4_data %>%
             filter(howrecent > 3)))
summary(lm(org_competent ~ condition * howrecent, manipCWV.4_data %>%
             filter(howrecent > 3)))
summary(lm(gen_competent ~ condition * howrecent, manipCWV.4_data %>%
             filter(howrecent > 3)))
summary(lm(org_competent ~ condition * howrecent, manipCWV.4_data %>%
